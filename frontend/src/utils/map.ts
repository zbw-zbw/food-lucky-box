...
export class MapService {
  ...
  public async getAddress(latitude: number, longitude: number): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.geocoder?.getAddress([longitude, latitude], (status: string, result: GeocodeResult) => {
  ...
  private saveCacheToStorage(): void {
    try {
      localStorage.setItem(CACHE_CONFIG.storageKey, JSON.stringify(this.searchCache));
    } catch (error) {
  ...
  private enforceCacheSize(): void {
    ...
    const sortedKeys = keys.sort(
      (a, b) => this.searchCache[a].lastAccessed - this.searchCache[b].lastAccessed
    );
  ...
  private async searchOSMRestaurants(location: Location, page: number = 1): Promise<Restaurant[]> {
      ...
      try {
        ...
        const restaurants = data.elements
          ...
          .map(node => ({
            ...
            rating: 0,
            distance: this.calculateDistance(latitude, longitude, node.lat, node.lon),
            photos: [],
        ...
        // 按距离排序并分页
        return restaurants.sort((a, b) => a.distance - b.distance).slice(offset, offset + limit);
      } catch (error) {
  ...
  private formatOSMAddress(tags: OSMNode['tags']): string {
    const parts = [tags['addr:street'], tags['addr:housenumber'], tags['addr:city']].filter(
      Boolean
    );
  ...
  // 将原来的高德地图搜索逻辑移到单独的方法
  private async searchAMapRestaurants(location: Location, page: number = 1): Promise<Restaurant[]> {
    if (!this.placeSearch) {
      throw new Error('地图服务未初始化');
    }
  ...
  public getRandomRestaurant(restaurants: Restaurant[], excludeId?: string): Restaurant {
