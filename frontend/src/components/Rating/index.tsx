    ...
    <span className={`${styles.rating} ${className || ''}`}>
      {stars.map((_, index) => {
        ...
        if (starValue - 0.5 <= rating) {
          return (
            <span key={index} className={styles.star}>
              <StarFill color="#ffd700" fontSize={16} style={{ clipPath: 'inset(0 50% 0 0)' }} />
              <StarOutline color="#999" fontSize={16} style={{ position: 'absolute' }} />
            </span>
          );
        }
        // 空星
      ...
      {value ? <span className={styles.score}>{value.toFixed(1)}分</span> : null}
    </span>
  );
};
